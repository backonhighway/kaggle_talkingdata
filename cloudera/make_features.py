...
def get_nunique(df: pd.DataFrame, name: str, grouping:list, nunique_col:str):
    grouper = df.groupby(grouping)
    uni_col = name + "_" + nunique_col + "_nunique"
...
input_df = pd.read_csv('../input/train_day3.csv', nrows=1000*100, dtype=dtypes)
...
input_df.to_csv("../output/engineer_debug.csv", float_format='%.6f', index=False)
